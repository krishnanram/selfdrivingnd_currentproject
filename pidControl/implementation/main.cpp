...
bool findBestParamUsingTwiddle(PID pid, double cte);
 ...
 ****************************************************************************/
  ...
  if (argc == 9 ) {
      ...
      cout << "Usage ./pid kp ki kd speed_goal throttle_kp throttle_ki throttle_kd twiddleFlag" << endl;
      return -1;
  }
  ...
  pid_throttle.speed_goal = speed_goal ;


  h.onMessage([&pid_throttle, &pid,twiddleFlag](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
        ...
        if (s.str() != "") {
          ...
          std::string event = j[0].get<std::string>();
            ...
            if (twiddleFlag == true && pid.twiddle_counter % 100 == 0) {
                if ( findBestParamUsingTwiddle(pid,cte) ) {
...
bool findBestParamUsingTwiddle(PID pid, double cte) {
      ...
      cout << "Iteration" << pid.twiddle_counter <<
           "\t best error" <<  pid.twiddle_best_error <<
      ...
      for (int i = 0; i < 3; i++) {
        ...
        pid.UpdateError(cte) ;
        twiddle_current_error =  pid.mean_squared_error ;

        if (twiddle_current_error < pid.twiddle_best_error) {  //last update worked, keep going

          tune_parameters[i] *= 1.1 ;
          pid.twiddle_best_error = twiddle_current_error ;
        }
        else {
          ...
          if (twiddle_current_error < pid.twiddle_best_error) {
